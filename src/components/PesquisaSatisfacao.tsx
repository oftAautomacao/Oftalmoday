...
const PesquisaSatisfacao: React.FC = () => {
  ...
  const handleBatchAction = (action: 'marcar' | 'desmarcar') => {
    ...
    allRowsPacientes.forEach(p => {
      const caminho = `/OFT/45/pesquisaSatisfacao/site/aEnviar/${p.id}/Copiado`;
      if (action === 'marcar') {
  ...
  // Para pesquisa de satisfação, envia no dia seguinte à consulta (exceto segunda)
  const calcularDiaEnvio = (dataStr: string): Date | null => {
    ...
    if (diaEnvio.getDay() === 1) return null; // Segunda não tem expediente
    return diaEnvio;
  };
  ...
  useEffect(() => {
    ...
    if (naoTemFiltros && database && (metricasEnvio.comTemplate > 0 || metricasEnvio.semTemplate > 0)) {
      ...
      const metricasRef = ref(database, `/OFT/45/pesquisaSatisfacao/metricasEnvio/${hoje}`);
      update(metricasRef, {
