...
const ReconfirmacaoPacientes: React.FC = () => {
  ...
  const [tabAtiva, setTabAtiva] = useState(0); // 0: Pacientes, 1: Erros, 2: Informações de Envio
  ...
  const handleBatchAction = (action: 'marcar' | 'desmarcar') => {
    ...
    rowsPacientes.forEach(p => {
      const caminho = `/OFT/45/reconfirmacaoPacientes/site/aEnviar/${p.id}/Copiado`;
      if (action === 'marcar') {
  ...
  // Para reconfirmação, envia no mesmo dia do agendamento (cedo)
  const calcularDiaEnvio = (dataStr: string): Date | null => {
    ...
    return new Date(ano, mes - 1, dia); // Mesmo dia
  };
  ...
  useEffect(() => {
    ...
    if (naoTemFiltros && database && (metricasEnvio.comTemplate > 0 || metricasEnvio.semTemplate > 0)) {
      ...
      const metricasRef = ref(database, `/OFT/45/reconfirmacaoPacientes/metricasEnvio/${hoje}`);
      update(metricasRef, {
